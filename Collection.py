...
# @Author  : CaoQixuan
# @File    : Collection.py
# @Description : git from  https://github.com/luo3300612/Visualizer  被装饰的模型一定要在装饰器激活之后导入！！
...
def grid_show(to_shows, cols, title=""):
...
def visualize_heads(att_map, cols, title=""):
    to_shows = []
    ...
    grid_show(to_shows, cols=cols, title=title)
...
def visualize_grid_to_grid_with_cls(att_map, grid_index, image, grid_size=14, alpha=0.5, title=""):
    if not isinstance(grid_size, tuple):
        grid_size = (grid_size, grid_size)
...
def visualize_grid_to_grid(att_map, grid_index, image, grid_size=14, alpha=0.5, title=""):
    if not isinstance(grid_size, tuple):
        grid_size = (grid_size, grid_size)
