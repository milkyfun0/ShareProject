...
def trans_token_embedding(static_dict, tokenizer_path, vocab_path, special_token_trans):
    ...
    def function(word):
        ...
        token = torch.mean(token_embedding(input_ids)[:, 1:-1], dim=1)
        return token

    new_token_embedding = torch.cat([function(word) for word in vocab], dim=0)
    static_dict["token_embedding.weight"] = new_token_embedding
    return static_dict


def load_clip_from_json(opt, writer=None):
    ...
    with open(opt["model"]["Bert"]["config_path"] + "tokenizer_config.json", 'r') as f:
        bert_cfg = json.load(f)

    with open(opt["model"]["CLIP"]["config_path"] + "config.json", 'r') as f:
    ...
    if opt["model"]["CLIP"]["pre_train"]:
        weights_map = trans_standard_static_dict(
            torch.load(opt["model"]["CLIP"]["config_path"] + "/" + model_cfg["name"]))
        weights_map = trans_token_embedding(weights_map, "models_data/base_clip/tokenizer",
                                            opt["model"]["Bert"]["config_path"] + "vocab.txt", trans_dict)
